        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAEBTxtoOIU0zS4G_1y3FmDswhXhXVVOuAACnhAAAn8WWFeT1TSYg_i9KzYE") 
...
async def refer(bot, message):
    ...
    m=await message.reply_sticker("CAACAgQAAxkBAAEBTvtoODF02IGNXeT2LI0uq5nGq_E3YwACMgMAAoY5uSA7pG7dj7DpjDYE")      
    await m.delete()
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
